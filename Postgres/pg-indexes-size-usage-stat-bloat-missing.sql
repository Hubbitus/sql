...
SELECT
	...
	END AS UNIQUE,
	SUM(
		CASE WHEN number_of_columns = 1 THEN 1
			ELSE 0
		END
	) AS single_column,
	SUM(
...
LEFT OUTER JOIN
	(
		...
		FROM pg_index
		GROUP BY indrelid
	) x ON pg_class.oid = x.indrelid
LEFT OUTER JOIN
	(
		SELECT
			c.relname AS ctablename, ipg.relname AS indexname, x.indnatts AS number_of_columns
		FROM
			pg_index x
			JOIN pg_class c ON c.oid = x.indrelid
			JOIN pg_class ipg ON ipg.oid = x.indexrelid
	) AS foo ON pg_class.relname = foo.ctablename
WHERE 
...
WHERE idstat.idx_scan < 200
	AND indexdef !~* 'unique'
ORDER BY times_used, index_size DESC, idstat.relname, indexrelname;
